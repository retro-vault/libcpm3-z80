        ...
        ;; -----------------------------------------
        ;; calls cp/m bdos
        ;; return: A (__sdcccall(1) 8-bit return register)
        ;; affect:  af, bc, de, hl
_bdos::
        push    ix                      ; store ix
        ...
        ;; return: DE = p (__sdcccall(1) 16-bit return register)
        ;; stores BDOS result registers A, B and HL into *p
        ;; affect:  af, bc, de, hl
_bdosret::
        ;; classic bdos call!
        push    ix
        call    rawbdos
		;; preserve BDOS HL result in DE while HL is used as a pointer.
		ex      de, hl      
        ...
        inc     hl
        ld      (hl),d
        ;; return p in DE for __sdcccall(1)
		ld      e,3(ix)
		ld      d,4(ix)
